...
const TemplateGeneratorLayout: React.FC = () => {
	...
	function getPromptForChoice(choice: string): string {
		switch (choice) {
			case "occasion":
				return `Create a JSON template for a personalized greeting for customer name ${customerName}  from an insurance company named Qoala.
			...
			case "discount":
				return `Create a JSON template for a discount offered by Qoala, an insurance company,. Additionally, generate an image that is free of any text, digits,no alphabet just image or company-specific branding, but is related to the concept of insurance. The JSON structure should be as follows: ${JSON.stringify(
					jsonTemplate[1]
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		console.log("textMessagesObj", textMessagesObj)

		localStorage.setItem(`message-${uniqueId}`, JSON.stringify(textMessagesObj))
